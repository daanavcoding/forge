...
assert.match(commitSkill, /one project-instructions file that the repository or active\s+Agent Plugins host has declared applicable/);
assert.match(commitSkill, /FORGE_PROJECT_CONTEXT/);
assert.match(commitSkill, /Stage only that scope/);
assert.match(commitSkill, /By default, authored or rewritten prose in `README\*\.md` and applicable project\s+instructions is English only/);
assert.match(commitSkill, /Follow an explicit language request/);
assert.match(commitSkill, /smallest durable, evidence-based guidance/);
assert.match(commitSkill, /Do not add skill names, README content, history/);
assert.match(commitSkill, /Inspect the root `README\.md` before staging every time/);
assert.match(commitSkill, /If the task changes user-facing\s+behavior or documentation, update only stale lines/);
assert.match(commitSkill, /preserving unrelated\s+edits/);
assert.match(commitSkill, /brief, descriptive English imperative commit subject/);
assert.match(commitSkill, /short, descriptive GitHub-style topic branch/);
assert.match(commitSkill, /`<type>\/\<short-description>`/);
assert.match(commitSkill, /`feat`, `fix`, `chore`/);
assert.match(commitSkill, /`feat\/add-export`/);
assert.match(commitSkill, /Do not add a `forge\/` prefix/);
assert.match(commitSkill, /pull request only when the user explicitly requests one/);
assert.match(commitSkill, /Mention opening a pull request into `<base>` only when the user requested/);
assert.match(commitSkill, /If no pull request was requested, do not look for or create one/);
assert.match(commitSkill, /Do not choose a file from a named coding-agent provider/);
...
assert.match(commitSkill, /original commit request is not this approval/);
assert.match(commitSkill, /return its\s+actual\s+URL/);
assert.match(commitSkill, /Never merge unless explicitly asked/);
